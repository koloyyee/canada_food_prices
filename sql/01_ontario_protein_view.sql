...
create view view_ontario_protein_prices as 
-- 1. farm products prices
with farm_mapped as (
select 'farm' as stage, month, price_per_unit, uom, product,
case
...
-- retail prices 
retail_mapped as (
	select 'retail' as stage, month, product, uom, price_per_unit,
	case 
...
select 
month, stage, product,  protein, price_per_unit, uom
from farm_mapped
where protein is not null

union all

select month, stage, product, protein, price_per_unit, uom
from retail_mapped 
where protein is not null 
)
-- 4. Normalise price per unit and uom
select month, stage, protein, product, price_per_unit, uom,
case
